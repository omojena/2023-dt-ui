<template>
  <div class="content-page">
    <section>
      <top-bar is_back page-title="Tipos de Tarea"/>
      <div class="mt-3 bg-withe p-5">
        <section v-if="!desserts">
          <skeleton-table/>
        </section>
        <section v-else>
          <h-list :go-to-url="addUrl" :records="desserts.count" :per-page="perPage" class="mb-6"/>
          <b-table
            ...
            <b-table-column field="count" label="Decripción" v-slot="props">
              <span class="col">
               {{ props.row.nombre }}
              </span>
            </b-table-column>
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formEditTypeTask', params:{id:props.row.id}}">
                  <div class="mx-10">
...
export default {
  name: 'LTTask',
  components: {SkeletonTable, HList, BTableColumn, BTable, TopBar},
  comments: {},
  data() {
    return {
      addUrl: '/type/tasks/form/',
      search: '',
      isEmpty: false,
      isPaginated: false,
  ...
  methods: {
    ...
    open_modal_delete: function (item) {
      let uri_delete = URLS.api_nomvalue + item.id + '/';
      let uri_get_data = URLS.api_type_task_nomvalue;
      let action = item.deleted ? 'recuperar': 'eliminar';
      let title = item.deleted ? 'Recuperar': 'Eliminando';
      this.$buefy.dialog.confirm({
        title: `${title} Tipo de Tarea`,
        message: `Estás seguro que desea ${action} el tipo de Tarea "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setTypeTask(resolve)
        }
      })
    },
    getTypeTAsk() {
      this.query = `?page=${this.paginate.current}`
      this.fetchTypeTask(this.query)
          .then(value => {
            this.desserts = value
          })
    }
