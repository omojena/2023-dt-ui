<template>
  <div class="content-page">
    <section>
      <top-bar is_back page-title="Tipos de Incidencias"/>
        ...
        <section v-else>
          ...
          <b-table
            ...
            <b-table-column field="nombre" label="Nombre" v-slot="props">
               <b-tooltip multilined :label="props.row.nombre">
              <span class="col">{{ props.row.nombre | truncate(50, '...')}}</span>
            </b-tooltip>
            </b-table-column>
         ...
         <b-table-column field="descripcion" label="Descripcion" v-slot="props">
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formEditTypeIncidents', params:{id:props.row.id}}">
                  <div class="mx-10">
...
<script>
import {
  api_nomvalue, api_tipo_incidencia_nomvalue
} from "@/urls";
...
export default {
  ...
  methods: {
    ...mapMutations({
      ...
      setTypeIncidents: 'SET_TYPE_INCIDENTS'
    ...
    open_modal_delete: function (item) {
      ...
      let uri_get_data = api_tipo_incidencia_nomvalue;
      let action = item.deleted ? 'recuperar': 'eliminar';
      let title = item.deleted ? 'Recuperar': 'Eliminando';
      this.$buefy.dialog.confirm({
        title: `${title} Tipo de Incidencia`,
        message: `Estás seguro que desea ${action} el tipo de incidencia "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setTypeIncidents(resolve);
        }
      })
    },
    getTypeIncidents() {
      this.query = `?page=${this.paginate.current}`
      this.fetchTypeIncidents(this.query)
          .then(value => {
            this.desserts = value
          })
    }
