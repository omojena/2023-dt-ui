        ...
        <section v-else>
          ...
          <b-table
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formEditPosition', params:{id:props.row.id}}">
                  <div class="mx-10">
  ...
  methods: {
    ...
    open_modal_delete: function (item) {
      ...
      this.$buefy.dialog.confirm({
        title: `${title} Cargo`,
        message: `Estás seguro que desea ${action} el cargo "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setPositions(resolve)
        }
      })
    },
    getPositions() {
      this.query = `?page=${this.paginate.current}`
      this.fetchPositions(this.query)
          .then(value => {
            this.desserts = value
          })
    }
