<template>
  <div class="content-page">
    <section>
      <top-bar is_back page-title="Tipos de Plazas"/>
        ...
        <section v-else>
          ...
          <b-table
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formEditTypeWorkplace', params:{id:props.row.id}}">
                  <div class="mx-10">
...
export default {
  ...
  methods: {
    ...mapMutations({
      ...
      setTypeWorkplace: 'SET_TYPE_WORKPLACE'
    }),
    ...mapActions({
      confirmDelete: 'CONFIRM_DELETE',
      fetchTypeWorkplace: 'FETCH_TYPE_WORKPLACE'
    }),
    ...
    open_modal_delete: function (item) {
      ...
      let uri_get_data = URL.api_tipo_plaza_nomvalue;
      let action = item.deleted ? 'recuperar': 'eliminar';
      let title = item.deleted ? 'Recuperar': 'Eliminando';
      this.$buefy.dialog.confirm({
        title: `${title} Tipo de Plaza`,
        message: `Estás seguro que desea ${action} el tipo de plaza "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setTypeWorkplace(resolve)
        }
      })
    },
    getTypeWorkplace() {
      this.query = `?page=${this.paginate.current}`
      this.fetchTypeWorkplace(this.query)
          .then(value => {
            this.desserts = value
          })
    }
