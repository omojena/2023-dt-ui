<template>
  <div class="content-page">
    <section>
      <top-bar page-title="Tipos de Cargos"/>
        ...
        <section v-else>
          ...
          <b-table
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formEditTypePosition', params:{id:props.row.id}}">
                  <div class="mx-10">
...
<script>
import {
  api_nomvalue, api_tipo_cargo_nomvalue
} from "@/urls";
...
export default {
  ...
  methods: {
    ...mapMutations({
      ...
      setTypePositions: 'SET_TYPE_POSITIONS'
    }),
    ...mapActions({
      confirmDelete: 'CONFIRM_DELETE',
      fetchTypePositions: 'FETCH_TYPE_POSITIONS'
    }),
    viewAll() {
    ...
    open_modal_delete: function (item) {
      ...
      let uri_get_data = api_tipo_cargo_nomvalue;
      let action = item.deleted ? 'recuperar': 'eliminar';
      let title = item.deleted ? 'Recuperar': 'Eliminando';
      this.$buefy.dialog.confirm({
        title: `${title} Tipo de Cargo`,
        message: `Estás seguro que desea ${action} el tipo de cargo "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setTypePositions(resolve)
        }
      })
    },
    getTypePositions() {
      this.query = `?page=${this.paginate.current}`
      this.fetchTypePositions(this.query)
          .then(value => {
            this.desserts = value
          })
    }
