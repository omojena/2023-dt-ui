<template>
  <div class="content-page">
    <section>
      <top-bar page-title="Clasificación de Personas"/>
        ...
        <section v-else>
          ...
          <b-table
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formEditClassificationPerson', params:{id:props.row.id}}">
                  <div class="mx-10">
...
<script>
import {
  api_clasificacion_persona_nomvalue,
  api_nomvalue
} from "@/urls";
...
export default {
  name: 'LPersonClassification',
  components: {SkeletonTable, BTableColumn, BTable, HList, TopBar},
  comments: {},
  data() {
    return {
      search: '',
      addUrl: '/type/person-classification/form/',
      isEmpty: false,
      isPaginated: false,
      currentPage: 1,
      perPage: 5,
  ...
  methods: {
    ...mapMutations({
      ...
      setClassificationPerson: 'SET_CLASSIFICATION_PERSON'
    }),
    ...mapActions({
      confirmDelete: 'CONFIRM_DELETE',
      fetchClassificationPerson: 'FETCH_CLASSIFICATION_PERSON'
    }),
    ...
    open_modal_delete: function (item) {
      ...
      let uri_get_data = api_clasificacion_persona_nomvalue;
      let action = item.deleted ? 'recuperar': 'eliminar';
      let title = item.deleted ? 'Recuperar': 'Eliminando';
      this.$buefy.dialog.confirm({
        title: `${title} Clasificación de Persona`,
        message: `Estás seguro que desea ${action} el la clafificación "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setClassificationPerson(resolve)
        }
      })
    },
    getClassificationPerson() {
      this.query = `?page=${this.paginate.current}`
      this.fetchClassificationPerson(this.query)
          .then(value => {
            this.desserts = value
          })
    }
