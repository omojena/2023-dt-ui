<template>
  <div class="content-page">
    <section>
      <top-bar is_back page-title="Tipos de Contrato"/>
        ...
        <section v-else>
          ...
          <b-table
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formEditTypeContract', params:{id:props.row.id}}">
                  <div class="mx-10">
...
<script>
import {api_nomvalue, api_tipo_contrato_nomvalue} from "@/urls";
...
export default {
  ...
  methods: {
    ...mapMutations({
      ...
      setTypeContract: 'SET_TYPE_CONTRACT'
    }),
    ...mapActions({
      confirmDelete: 'CONFIRM_DELETE',
      fetchTypeContract: 'FETCH_TYPE_CONTRACT'
    }),
    ...
    open_modal_delete: function (item) {
      ...
      let uri_get_data = api_tipo_contrato_nomvalue;
      let action = item.deleted ? 'recuperar': 'eliminar';
      let title = item.deleted ? 'Recuperar': 'Eliminando';
      this.$buefy.dialog.confirm({
        title: `${title} Tipo de Contrato`,
        message: `Estás seguro que desea ${action} el tipo de contrato "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setTypeContract(resolve)
        }
      })
    },
    getTypeContract() {
      this.query = `?page=${this.paginate.current}`
      this.fetchTypeContract(this.query)
          .then(value => {
            this.desserts = value
          })
    }
