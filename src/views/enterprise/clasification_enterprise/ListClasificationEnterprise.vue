<template>
  <div class="content-page">
    <section>
      <top-bar is_back page-title="Clasificación de Empresas"/>
        ...
        <section v-else>
          ...
          <b-table
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formEditClassificationEnterprise', params:{id:props.row.id}}">
                  <div class="mx-10">
...
<script>
import {
  api_clasificacion_empresa_nomvalue,
  api_nomvalue
} from "@/urls";
...
export default {
  name: 'LClassificationEnterprise',
  components: {SkeletonTable, BTable, BTableColumn, HList, TopBar},
  comments: {},
  data() {
    return {
      search: '',
      addUrl: '/type/enterprise-classification/form/',
      isEmpty: false,
      isPaginated: false,
      currentPage: 1,
      perPage: 5
  ...
  computed: {
    ...mapGetters({
      desserts: 'GET_CLASSIFICATION_ENTERPRISE'
    })
  },
  methods: {
    ...mapMutations({
      ...
      setClassificationEnterprise: 'SET_CLASSIFICATION_ENTERPRISE'

    }),
    ...mapActions({
      confirmDelete: 'CONFIRM_DELETE',
      fetchClassificationEnterprise: 'FETCH_CLASSIFICATION_ENTERPRISE'
    }),
    ...
    open_modal_delete: function (item) {
      ...
      let uri_get_data = api_clasificacion_empresa_nomvalue;
      let action = item.deleted ? 'recuperar': 'eliminar';
      let title = item.deleted ? 'Recuperar': 'Eliminando';
      this.$buefy.dialog.confirm({
        title: `${title} Clasificación de Empresa`,
        message: `Estás seguro que desea ${action} la clasificación "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setClassificationEnterprise(resolve)
        }
      })
    },
  },
  created() {
    this.fetchClassificationEnterprise();
  },
