<template>
  <div class="content-page">
    <section>
      <top-bar page-title="Tipos de Producto"/>
        ...
        <section v-else>
          ...
          <b-table
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formTypeProduct', params:{id:props.row.id}}">
                  <div class="mx-10">
...
export default {
  ...
  methods: {
    ...mapMutations({
      ...
      setTypeProductList: 'SET_PRODUCTS_TYPE_LIST'
    }),
    ...mapActions({
      confirmDelete: 'CONFIRM_DELETE',
      fetchProductList: 'FETCH_TYPE_PRODUCT'
    }),
    ...
    open_modal_delete: function (item) {
      ...
      let uri_get_data = URL.api_get_type_product;
      let action = item.deleted ? 'recuperar': 'eliminar';
      let title = item.deleted ? 'Recuperar': 'Eliminando';
      this.$buefy.dialog.confirm({
        title: `${title} Tipo de Producto`,
        message: `Estás seguro que desea ${action} el tipo de producto "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setTypeProductList(resolve)
        }
      })
    },
    getTypeProducts() {
      this.query = `?page=${this.paginate.current}`
      this.fetchProductList(this.query)
          .then(value => {
            this.desserts = value
          })
    }
