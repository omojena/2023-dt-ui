        ...
        <section v-else>
          ...
          >
            ...
            <b-table-column field="action" label="Acciones" v-slot="props">
              <div class="is-flex">
                <router-link :to="{name: 'formSupplies', params:{id:props.row.id}}">
                  <div class="mx-10">
...
export default {
  ...
  data() {
    return {
      search: '',
      checkedRows: [],
      addUrl: '/warehouse/supplies/form/',
      isEmpty: false,
      isPaginated: false,
      currentPage: 1,
      perPage: 5,
  ...
  methods: {
    ...mapMutations({
      ...
      setSuppliesList:'SET_SUPPLIES_LIST'
    }),
    ...mapActions({
      confirmDelete: 'CONFIRM_DELETE',
      fetSuppliesLIst:'FETCH_SUPPLIES'
    }),
    ...
    open_modal_delete: function (item) {
      let uri_delete = api_insumos + item.id + '/';
      let uri_get_data = api_insumos;
      let action = item.deleted ? 'recuperar': 'eliminar';
      let title = item.deleted ? 'Recuperar': 'Eliminando';
      this.$buefy.dialog.confirm({
        title: `${title} Activo`,
        message: `Estás seguro que desea ${action} el insumo "${item.nombre}"`,
        ...
        onConfirm: async () => {
          ...
          this.setSuppliesList(resolve)
        }
      })
    },
    getSupplies(){
      this.query = `?page=${this.paginate.current}`
      this.fetSuppliesLIst(this.query)
          .then(value => {
            this.desserts = value
          })
    }
